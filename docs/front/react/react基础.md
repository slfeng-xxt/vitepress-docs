...
- `setState` 是 React 类组件中的一个方法，用于更新组件的状态。当状态发生变化时，组件会重新渲染。
- `setState` 接收一个对象或函数作为参数，用于更新状态。如果参数是一个对象，React 会将对象的属性合并到当前状态中。
如果参数是一个函数，函数的返回值会被合并到当前状态中。
- 如果改变状态的代码处于某个HTML元素的事件中，则是异步操作，否则同步
...
class MyComponent extends React.Component {
  constructor(props) {
    ...
    if (this.state.count === 5) {clearTimeout(timer);
  }

  handleClick = () => {

    // setState 异步操作说明：在事件处理函数中，setState 是异步操作，即调用 setState 后，立即打印状态值，状态值不会立即更新。
    // this.setState({ count: this.state.count + 1 });
    // console.log(this.state.count); // 0 =====》异步操作，先执行console.log，再执行setState

    // setState 回调参数：在调用 setState 后，可以传递一个回调函数，该回调函数会在状态更新后执行。回调函数的参数是更新后的状态值。
    // this.setState({ count: this.state.count + 1 }, () => {
    //   console.log(this.state.count); // 1
    // });
...
- **受控组件**：受控组件是由 React 控制其值的组件。在受控组件中，表单元素的值由 React 的状态管理，并且通过事件处理函数来更新状态。例如，可以使用 `value` 属性来设置输入框的值，并使用 `onChange` 事件处理函数来更新状态。推荐使用！！！
...
- **非受控组件**：非受控组件是由 DOM 控制其值的组件。在非受控组件中，表单元素的值由 DOM 管理，并且可以通过 `ref` 来访问表单元素的值。例如，可以使用 `ref` 来获取输入框的值。文件上传。。。
...
class UncontrolledComponent extends React.Component {
  constructor(props) {
    super(props);
    this.inputRef = React.createRef();// 创建一个ref对象
  }

  handleClick = () => {
    console.log(this.inputRef.current.value);// 通过ref对象获取输入框的值
  };

  render() {
    return <input type="text" ref={this.inputRef} defaultValue="xxxx"/>;
  }
}
```
...
- `constructor()`：组件的构造函数，在组件创建时被调用。可以在构造函数中初始化组件的状态和绑定事件处理函数。不要在构造函数中调用 `setState()` 方法。
- `static getDerivedStateFromProps()`：在组件挂载和更新时被调用，用于根据 props 更新组件的状态。这个方法应该返回一个对象，表示新的状态，或者返回 null 表示状态不需要更新。
- `render()`：组件的渲染方法，在组件挂载和更新时被调用。这个方法应该返回一个 React 元素，表示组件的 UI 结构。这里严禁使用 setState()，否则会导致无限循环。
